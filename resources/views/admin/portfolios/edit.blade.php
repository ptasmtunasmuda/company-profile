...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Edit Portfolio Project</h1>
            <p class="mt-1 text-sm text-gray-600">Update {{ $portfolio->title }}</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('portfolio.detail', $portfolio->slug) }}" target="_blank" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
    ...
    <div class="bg-white shadow rounded-lg">
        <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" required
                                ...
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('description') border-red-500 @enderror">{{ old('description', $portfolio->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="15" required
                                ...
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('content') border-red-500 @enderror">{{ old('content', $portfolio->content) }}</textarea>
            ...
            <div class="flex justify-between pt-6 border-t border-gray-200">
                <form action="{{ route('admin.portfolios.destroy', $portfolio) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this portfolio? This action cannot be undone.')">
                    @csrf
                    @method('DELETE')
